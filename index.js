...
const working = () => {
  ...
  pixels.forEach(pxl => { 
    pxl.addEventListener('mouseover', (option) => {
      let crntClr = getComputedStyle(pxl, null).getPropertyValue('background-color');
      switch(currentMode){
        case 'black':
          option.target.style.backgroundColor = 'rgba(0,0,0)';
          break;
        case 'colors':
          option.target.style.backgroundColor = randomColor();
          break;
        case 'shading':
          option.target.style.backgroundColor = shading(crntClr);
      }
    });
  });
}
